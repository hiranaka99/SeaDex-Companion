...
export default function OperationCenter({ status, scanCompleted, bulk, onRetryScan, onCancelScan, onOpenLibrary, onOpenConfig, onDismissBulk, onDismissScan }: Props) {
  ...
  return (
    <section className="sticky top-0 z-40 mb-5 space-y-2 rounded-2xl border border-line-strong bg-panel-raised/95 p-3 shadow-card backdrop-blur-xl max-[900px]:top-16" aria-label="Application operations">
      {scanVisible && (
        <div className={cx('rounded-xl border px-3.5 py-3', (status.error || status.cancelled) && !status.running ? 'border-bad/35 bg-bad/8' : scanCompleted && !status.running ? 'border-good/35 bg-good/8' : 'border-accent/30 bg-accent/7')} role={(status.error || status.cancelled) && !status.running ? 'alert' : 'status'} aria-live="polite">
