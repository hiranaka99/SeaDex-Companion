...
interface Props { config: Config | null; status: Status; username: string; onAccountUpdated: (username: string) => void; onSaved: () => void; onScannedDataCleared: () => void }
...
function ScheduleEditor({ schedule, nextCheck, onChange }: { schedule: ScanSchedule; nextCheck: number | null; onChange: (changes: Partial<ScanSchedule>) => void }) {
  const nextScan = schedule.enabled && nextCheck
    ? new Intl.DateTimeFormat(undefined, { dateStyle: 'full', timeStyle: 'short', timeZone: schedule.timezone }).format(new Date(nextCheck * 1000))
    : null
...
export default function ConfigTab({ config, status, username, onAccountUpdated, onSaved, onScannedDataCleared }: Props) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    for (const key of Object.keys(form)) { if (key.endsWith('_configured')) continue; if (key === 'notify_enabled') data[key] = !!form[key]; else data[key] = form[key] || '' }
  ...
  <ScheduleEditor schedule={schedule} nextCheck={status.next_check} onChange={setSchedule}/>
