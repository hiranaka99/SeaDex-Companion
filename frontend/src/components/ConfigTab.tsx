...
interface Props { config: Config | null; username: string; onAccountUpdated: (username: string) => void; onSaved: () => void; onScannedDataCleared: () => void }
...
export default function ConfigTab({ config, username, onAccountUpdated, onSaved, onScannedDataCleared }: Props) {
  ...
  useEffect(() => {
    ...
    const totalMinutes = Number(next.autocheck_minutes || 0)
    next.autocheck_hours = Math.floor(totalMinutes / 60)
    next.autocheck_minutes = totalMinutes % 60
  ...
  const submit = async (event: FormEvent) => {
    ...
    for (const key of Object.keys(form)) { if (key.endsWith('_configured') || key === 'autocheck_hours') continue; if (key === 'notify_enabled') data[key] = !!form[key]; else if (key === 'autocheck_minutes') data[key] = Math.max(0, Math.round(Number(form.autocheck_hours || 0) * 60 + (form[key] === '' ? 0 : Number(form[key])))); else data[key] = form[key] || '' }
  ...
  <section className="rounded-2xl border border-line bg-panel p-5"><header className="mb-5 flex items-center gap-3"><span className="grid size-10 place-items-center rounded-xl bg-canvas-soft text-accent-bright"><Icon name="user" size={19}/></span><div><h2 className="m-0 text-base font-extrabold">Administrator account</h2><p className="mt-1 mb-0 text-xs text-muted">Change the login name or password. Saving revokes every other active session.</p></div></header><div className="grid grid-cols-2 gap-4 max-[700px]:grid-cols-1"><label className="flex flex-col gap-1.5"><span className="text-xs font-bold text-muted">Username</span><input className={cx(control, 'w-full bg-canvas-soft')} value={account.username} autoComplete="username" onChange={(event) => setAccount((current) => ({ ...current, username: event.target.value }))}/></label><label className="flex flex-col gap-1.5"><span className="text-xs font-bold text-muted">Current password</span><input className={cx(control, 'w-full bg-canvas-soft')} type="password" value={account.currentPassword} autoComplete="current-password" onChange={(event) => setAccount((current) => ({ ...current, currentPassword: event.target.value }))}/></label><label className="flex flex-col gap-1.5"><span className="text-xs font-bold text-muted">New password <span className="font-normal text-muted-dim">(optional)</span></span><input className={cx(control, 'w-full bg-canvas-soft')} type="password" value={account.newPassword} autoComplete="new-password" placeholder="At least 10 characters" onChange={(event) => setAccount((current) => ({ ...current, newPassword: event.target.value }))}/></label><label className="flex flex-col gap-1.5"><span className="text-xs font-bold text-muted">Confirm new password</span><input className={cx(control, 'w-full bg-canvas-soft')} type="password" value={account.confirmPassword} autoComplete="new-password" onChange={(event) => setAccount((current) => ({ ...current, confirmPassword: event.target.value }))}/></label></div><div className="mt-4 flex justify-end"><button type="button" className={cx(buttonBase, 'border-accent/35 bg-accent/10 text-accent-bright hover:bg-accent/18')} onClick={() => void saveAccount()} disabled={savingAccount}>{savingAccount ? <span className="size-4 animate-spin rounded-full border-2 border-accent/30 border-t-accent"/> : <Icon name="check" size={16}/>} {savingAccount ? 'Updating account…' : 'Update account'}</button></div></section>
